...
function frenumDir(repo) {
  const fDir = join(repo, '.frenum');
  const gDir = join(repo, '.genesis');
  if (existsSync(fDir)) return fDir;
  if (existsSync(gDir)) return gDir;
  return fDir;
}
...
function loadState(repo) {
  ...
  if (!existsSync(path)) throw new Error(`no Frenum state at ${path}; run init, adopt --write, or migrate --write`);
...
function listFiles(root, { includeFrenum = false } = {}) {
  ...
  function walk(dir) {
    for (const name of readdirSync(dir).sort()) {
      ...
      if (!includeFrenum && (name === '.frenum' || name === '.genesis')) continue;
...
function discover(repo) {
  ...
  return {
    ...
    commands,
    legacy_genesis: (existsSync(join(repo, '.frenum')) || existsSync(join(repo, '.genesis'))) && !existsSync(statePath(repo)),
...
function renderPlan(repo, state) {
  ...
  const tasks = state.tasks.filter((task) => task.id !== 'SPEC-1');
  const lines = [
    '# Implementation plan', '', '> Generated from project.json; do not edit this file.', '',
...
function renderKickoff(repo, state) {
  ...
  const lines = [
    ...
    '',
    '> Generated from project.json; do not edit this file.',
    '',
    ...
    '## Resume',
    '',
    '1. Read this file and the active task in project.json.',
...
<!-- frenum:end -->`;
  ...
  for (const name of selected) {
    ...
    if (!before.includes('<!-- frenum:start -->') && !before.includes('<!-- genesis:start -->')) atomicWrite(path, `${before}${before && !before.endsWith('\n') ? '\n' : ''}${before ? '\n' : ''}${block}\n`);
  }
...
function commandGate(parsed) {
  ...
  for (const gate of task.gates) {
    ...
    gate.evidence = { path: `${frenumDir(repo).replace(repo, '').replace(/^[\\/]/, '')}/evidence/${id}-${gate.id}.json`, source_hash: proof.source_hash, hash: proof.hash, observed_at: proof.observed_at };
...
function commandMigrate(parsed) {
  ...
  if (!existsSync(dir)) throw new Error('no legacy .genesis or .frenum directory found');
  if (existsSync(statePath(repo))) {
    console.log('Frenum state already exists; migration is complete.');
    return;
  }

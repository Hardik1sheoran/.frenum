...
function fixture() {
  const root = mkdtempSync(join(tmpdir(), 'frenum-graph-'));
...
test('dry-run emits deterministic qualified graph without writing', () => {
  ...
  assert.throws(() => statSync(join(root, '.frenum', 'index', 'graph.json')));
});
...
test('--write creates views and leaves unchanged output untouched', async () => {
  ...
  const jsonPath = join(root, '.frenum', 'index', 'graph.json'), before = statSync(jsonPath).mtimeMs;
  assert.match(readFileSync(join(root, '.frenum', 'index', 'graph.dot'),'utf8'), /^digraph frenum/);
  assert.match(readFileSync(join(root, '.frenum', 'index', 'graph.html'),'utf8'), /Filter graph/);

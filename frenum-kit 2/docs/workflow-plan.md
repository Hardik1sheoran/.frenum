...
1. `.frenum/project.json` remains the only operational source of truth.
...
5. A generated `.frenum/PLAN.md` projects tasks from the ledger; it is never independently edited.
...
1. `frenum init . --workflow new-product --objective "..."` creates the ledger, `SPEC.md`, and an active specification task.
2. The agent interviews the human and records decisions, assumptions, constraints, and sourced knowledge.
3. `frenum spec check .` validates required sections and stable `FR-*`, `NFR-*`, and `AC-*` identifiers.
4. `frenum spec approve . --human NAME --reason "..."` binds human approval to the current specification hash and moves to planning.
5. Planning tasks reference approved requirement IDs through `--requirement` and name executable gates.
6. `frenum plan check .` fails closed on missing, unknown, or unproved requirements.
7. `frenum plan approve . --human NAME` activates the first task and moves to build.
...
- Existing Frenum v2 projects remain valid and can start the new-product workflow explicitly.
...
- Automatic requirement prose generation inside the CLI; the coding agent performs reasoning, Frenum records and gates it.
